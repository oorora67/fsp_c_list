...
class BoothGUI:
    ...
    def save_json(self):
        ...
        try:
            with open(path, "w", encoding="utf-8") as f:
                # 1要素1行で出力するためカスタム処理
                f.write("[\n")
                for i, item in enumerate(self.loaded_json):
                    line = json.dumps(item, ensure_ascii=False, separators=(",", ":"))
                    if i < len(self.loaded_json) - 1:
                        f.write(f" {line},\n")
                    else:
                        f.write(f" {line}\n")
                f.write("]")
    ...
    def __init__(self, master):
        ...
        # --- タレント編集ボタンを下部に横並びで統一配置 ---
        self.btns_frame_talent = tk.Frame(self.edit_frame_talent)
        self.btns_frame_talent.grid(row=3, column=0, columnspan=5, pady=8)
        self.update_btn_talent = tk.Button(
            self.btns_frame_talent,
            text="選択タレントタグを更新",
            width=18,
            command=lambda: self.update_selected("talent"),
        )
        self.update_btn_talent.pack(side="left", padx=8)
        ...
        self.add_btn_talent.pack(side="left", padx=8)
        ...
        self.delete_btn_talent.pack(side="left", padx=8)
        ...
        # --- ユニット編集ボタンを下部に横並びで統一配置 ---
        self.btns_frame_unit = tk.Frame(self.edit_frame_unit)
        self.btns_frame_unit.grid(row=4, column=0, columnspan=5, pady=8)
        self.update_btn_unit = tk.Button(
            self.btns_frame_unit,
            text="選択ユニットタグを更新",
            width=18,
            command=lambda: self.update_selected("unit"),
        )
        self.update_btn_unit.pack(side="left", padx=8)
        ...
        self.add_btn_unit.pack(side="left", padx=8)
        ...
        self.delete_btn_unit.pack(side="left", padx=8)
        ...
        # --- カテゴリ編集ボタンを下部に横並びで統一配置 ---
        self.btns_frame_category = tk.Frame(self.edit_frame_category)
        self.btns_frame_category.grid(row=3, column=0, columnspan=5, pady=8)
        self.update_btn_category = tk.Button(
            self.btns_frame_category,
            text="選択カテゴリタグを更新",
            width=18,
            command=lambda: self.update_selected("category"),
        )
        self.update_btn_category.pack(side="left", padx=8)
        ...
        self.add_btn_category.pack(side="left", padx=8)
        ...
        self.delete_btn_category.pack(side="left", padx=8)

        # グッズ編集フォーム
        self.edit_frame_goods = tk.LabelFrame(
            self.tab_goods, text="グッズ編集", padx=10, pady=10
        )
        self.edit_frame_goods.pack(fill="x", padx=10, pady=10)

        # 1行目: ID・名前・価格
        tk.Label(self.edit_frame_goods, text="ID:").grid(
            row=0, column=0, sticky="e", padx=2, pady=2
        )
        self.edit_id_goods = tk.Entry(self.edit_frame_goods, width=10)
        self.edit_id_goods.grid(row=0, column=1, sticky="w", padx=2, pady=2)
        tk.Label(self.edit_frame_goods, text="名前:").grid(
            row=0, column=2, sticky="e", padx=2, pady=2
        )
        self.edit_name_goods = tk.Entry(self.edit_frame_goods, width=30)
        self.edit_name_goods.grid(row=0, column=3, sticky="w", padx=2, pady=2)
        tk.Label(self.edit_frame_goods, text="価格:").grid(
            row=0, column=4, sticky="e", padx=2, pady=2
        )
        self.edit_price_goods = tk.Entry(self.edit_frame_goods, width=8)
        self.edit_price_goods.grid(row=0, column=5, sticky="w", padx=2, pady=2)
        ...
        self.listbox_category_select.grid(row=1, column=1, sticky="w", padx=2, pady=2)
        self.btn_add_category = tk.Button(
            self.edit_frame_goods,
            text="追加",
            command=self.add_selected_category,
            width=5,
        )
        self.btn_add_category.grid(row=1, column=2, sticky="w", padx=2, pady=2)
        self.edit_category_goods = tk.Entry(
            self.edit_frame_goods, width=25, state="readonly"
        )
        self.edit_category_goods.grid(row=1, column=3, sticky="w", padx=2, pady=2)
        ...
        self.listbox_talent_select.grid(row=2, column=1, sticky="w", padx=2, pady=2)
        self.btn_add_talent = tk.Button(
            self.edit_frame_goods,
            text="追加",
            command=self.add_selected_talent,
            width=5,
        )
        self.btn_add_talent.grid(row=2, column=2, sticky="w", padx=2, pady=2)
        self.edit_talent_goods = tk.Entry(
            self.edit_frame_goods, width=25, state="readonly"
        )
        self.edit_talent_goods.grid(row=2, column=3, sticky="w", padx=2, pady=2)
        ...
        self.listbox_unit_select.grid(row=3, column=1, sticky="w", padx=2, pady=2)
        self.btn_add_unit = tk.Button(
            self.edit_frame_goods, text="追加", command=self.add_selected_unit, width=5
        )
        self.btn_add_unit.grid(row=3, column=2, sticky="w", padx=2, pady=2)
        self.edit_generation_goods = tk.Entry(
            self.edit_frame_goods, width=25, state="readonly"
        )
        self.edit_generation_goods.grid(row=3, column=3, sticky="w", padx=2, pady=2)

        # 3行目: タグフラグ・商品URL・画像URL
        ...
        self.edit_tagflag_goods.grid(row=4, column=1, sticky="w", padx=2, pady=2)

        tk.Label(self.edit_frame_goods, text="商品URL:").grid(
            row=4, column=2, sticky="e", padx=2, pady=2
        )
        self.edit_url_goods = tk.Entry(self.edit_frame_goods, width=30)
        self.edit_url_goods.grid(row=4, column=3, sticky="w", padx=2, pady=2)

        tk.Label(self.edit_frame_goods, text="画像URL:").grid(
            row=4, column=4, sticky="e", padx=2, pady=2
        )
        self.edit_image_goods = tk.Entry(self.edit_frame_goods, width=30)
        self.edit_image_goods.grid(row=4, column=5, sticky="w", padx=2, pady=2)

        # 画像プレビュー
        self.goods_image_label = tk.Label(self.edit_frame_goods)
        self.goods_image_label.grid(row=2, column=5, padx=8, pady=8, sticky="n")

        # ボタン類
        # --- グッズ編集ボタンを下部に横並びで統一配置 ---
        self.btns_frame_goods = tk.Frame(self.edit_frame_goods)
        self.btns_frame_goods.grid(row=6, column=0, columnspan=6, pady=8)
        self.update_btn_goods = tk.Button(
            self.btns_frame_goods,
            text="選択グッズを更新",
            command=lambda: self.update_selected("goods"),
            width=18,
        )
        self.update_btn_goods.pack(side="left", padx=8)
        ...
        self.add_btn_goods.pack(side="left", padx=8)
        ...
        self.delete_btn_goods.pack(side="left", padx=8)
    ...
    def add_new_goods(self):
        ...
        url = self.edit_url_goods.get().strip()
    ...
    def on_tab_changed(self, event):
        ...
        try:
            ...
            return
        # グッズタブがindex=0なので修正
        is_goods = tab == 0  # 0:グッズ, 1:タレント, 2:ユニット, 3:カテゴリ
    ...
    def update_selected(self, kind=None):
        ...
        url_val = self.edit_url_goods.get().strip() if kind == "goods" else ""
    ...
    def add_to_list(self):
        ...
        if len(self.products) == 1:
            images = product.get("image", [{}])[idx].get("original", "")
            image_url = images
        else:
        ...
        # 商品URL欄にBoothのURLを必ずセット
        self.edit_url_goods.delete(0, tk.END)
        self.edit_url_goods.insert(0, self.url_entry.get().strip())
