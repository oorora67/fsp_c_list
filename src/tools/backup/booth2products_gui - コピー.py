...
class BoothGUI:
    ...
    def move_item_down(self, kind):
        ...
        self._init_treeview_drag_and_drop(
            self.tree_talent, "items_talent", self.refresh_listbox_all
        )
...
def booth_to_products(booth_json):
    ...
    # 並び替えボタン（カテゴリ）
    # （カテゴリタブの並び替えボタン初期化は__init__内に移動済み）


class BoothGUI:
    def _init_treeview_drag_and_drop(self, tree, items_attr, refresh_method):
        # Treeviewドラッグ＆ドロップ並び替え用
        if not hasattr(self, "_drag_data"):
            self._drag_data = {}

        def on_button_press(event, tree=tree):
            iid = tree.identify_row(event.y)
            if iid:
                tree.selection_set(iid)
                self._drag_data[tree] = {"item": iid, "from_idx": tree.index(iid)}
            else:
                self._drag_data[tree] = None

        def on_b1_motion(event, tree=tree):
            drag = self._drag_data.get(tree)
            if not drag or not drag.get("item"):
                return
            y = event.y
        ...
        def on_button_release(
            event, tree=tree, items_attr=items_attr, refresh_method=refresh_method
        ):
            drag = self._drag_data.get(tree)
            if not drag or not drag.get("item"):
                return
            ...
            if not target:
                ...
                to_idx = tree.index(target)
            if from_idx == to_idx:
                return
            item = items.pop(from_idx)
            # ドロップ先が元より下の場合、挿入位置を1つ下げる
            if to_idx > from_idx:
                to_idx -= 1
            items.insert(to_idx, item)
            refresh_method()
            # 並び替え後のTreeviewで新しい位置を選択
            children = tree.get_children()
            if 0 <= to_idx < len(children):
                tree.selection_set(children[to_idx])
                tree.focus(children[to_idx])
    ...
    def load_json(self):
        ...
        try:
            ...
            # items_*リストへの分配はrefresh_listbox_allで行う（重複防止のため）
            self.refresh_listbox_all()
    ...
    def __init__(self, master):
        ...
        self._init_treeview_drag_and_drop(
            self.tree_unit, "items_unit", self.refresh_listbox_all
        )
        ...
        self._init_treeview_drag_and_drop(
            self.tree_category, "items_category", self.refresh_listbox_all
        )
        ...
        self._init_treeview_drag_and_drop(
            self.tree_goods, "items_goods", self.refresh_listbox_all
        )
    ...
    # def on_listbox_select(self, kind):
    ...
    def add_to_list(self):
        ...
        # 既存リストに同じIDがあれば上書き
        for i, p in enumerate(self.loaded_json):
            if str(p.get("id")) == str(product["id"]):
                self.loaded_json[i] = product
                break
        else:
            self.loaded_json.append(product)
        self.refresh_listbox_all()
    ...
    def refresh_listbox_all(self):
        ...
        for p in self.loaded_json:
            pid = str(p.get("id", ""))
            # グッズはIDが数字のみなので、ID重複はitems_goodsで別途排除
            if pid.startswith("T"):
